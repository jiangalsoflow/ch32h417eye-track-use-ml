"""
Final verification of the complete JPEG decode fix.
"""
import numpy as np
import cv2
...
def idct_final(coeffs):
    """Final IDCT with correct scaling"""
    temp = [0] * 64
        ...
        for i in range(8):
            ...
            for k in range(8):
                ...
                if k == 0:
                    c = (c * 11585) >> 14
                s += COS_TABLE[i][k] * c
            val = (s >> 16) + 128  # >> 16 gives /4 scaling
            if val < 0: val = 0
            if val > 255: val = 255
            out[i * 8 + j] = val
    return out
...
Image.fromarray(output).save(r'E:\dvp-uart\Match\code\final_decode.png')
print(f"\nSaved final_decode.png")
