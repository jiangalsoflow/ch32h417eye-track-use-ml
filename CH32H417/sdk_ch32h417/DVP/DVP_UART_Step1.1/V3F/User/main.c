...
int main(void)
{
	...
	/* Print reset cause */
	{
		...
		if (RCC_GetFlagStatus(RCC_FLAG_IWDGRST) != RESET)  cause = "IWDG (watchdog)";
		else if (RCC_GetFlagStatus(RCC_FLAG_WWDGRST) != RESET) cause = "WWDG (window watchdog)";
		else if (RCC_GetFlagStatus(RCC_FLAG_SFTRST)  != RESET) cause = "SOFT (software)";
		else if (RCC_GetFlagStatus(RCC_FLAG_PORRST)  != RESET) cause = "POR (power-on)";
		else if (RCC_GetFlagStatus(RCC_FLAG_PINRST)  != RESET) cause = "PIN (external)";
		RCC_ClearFlag();
		printf("RST: %s\r\n", cause);
	}
	printf("SystemClk:%d\r\n", SystemClock);
