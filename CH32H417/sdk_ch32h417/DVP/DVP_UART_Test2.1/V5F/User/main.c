...
int main(void)
{
    ...
    if (ret == JPEG_DEC_OK) {
        ...
        /* --- Preprocessing: resize to 32x32, normalize, replicate to 3 channels --- */
        ...
        for (int i = 0; i < 6; i++) {
            ...
            send_byte('=');
            UINT32 score_int = (UINT32)(output[i] * 10000);
        ...
        send_u32(infer_time);
        send_str(" ticks\r\n");
        send_str("Preprocessed (first 32):");
        for (int i = 0; i < 32; i++) {
            UINT32 val_int = (UINT32)(input_buf[i] * 1000000);
            send_u32(val_int / 1000000);
            send_byte('.');
            UINT32 frac = val_int % 1000000;
            if (frac < 100000) send_byte('0');
            if (frac < 10000) send_byte('0');
            if (frac < 1000) send_byte('0');
            if (frac < 100) send_byte('0');
            if (frac < 10) send_byte('0');
            send_u32(frac);
