...
int main(void)
{
    ...
    while(1) {
        ...
        if (ret == JPEG_DEC_OK) {
            ...
            for (int y = 0; y < 32; y++) {
                for (int x = 0; x < 32; x++) {
                    int src_x = crop_x1 + CROP_SZ - 1 - (x * CROP_SZ / 32);
                    int src_y = crop_y1 + (y * CROP_SZ / 32);
            ...
            send_str(" ticks\r\n");
            send_str("CROP:");
            send_i32(crop_x1 + CROP_SZ / 2);
            send_byte(',');
            send_i32(crop_y1 + CROP_SZ / 2);
            send_str("\r\n");

            UINT32 cksum = 0;
            for (int i = 0; i < 32 * 32; i++)
                cksum += (UINT8)(input_buf[i * 3] * 255.0f);
            send_str("PREPROC_CKSUM:");
            send_u32(cksum);
            send_str("\r\n");
