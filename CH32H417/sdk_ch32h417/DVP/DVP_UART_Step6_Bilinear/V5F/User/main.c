...
static const UINT8 bl_x_idx[32] = {188,187,186,185, 183,182,181,180, 178,177,176,175, 173,172,171,170, 168,167,166,165, 163,162,161,160, 158,157,156,155, 153,152,151,150};
static const UINT8 bl_x_w[32]   = {224,160,96,32, 224,160,96,32, 224,160,96,32, 224,160,96,32, 224,160,96,32, 224,160,96,32, 224,160,96,32, 224,160,96,32};
static const UINT8 bl_y_idx[32] = {90,91,92,93, 95,96,97,98, 100,101,102,103, 105,106,107,108, 110,111,112,113, 115,116,117,118, 120,121,122,123, 125,126,127,128};
static const UINT8 bl_y_w[32]   = {32,96,160,224, 32,96,160,224, 32,96,160,224, 32,96,160,224, 32,96,160,224, 32,96,160,224, 32,96,160,224, 32,96,160,224};
...
int main(void)
{
    ...
    while(1) {
        ...
        if (ret == JPEG_DEC_OK) {
            ...
            for (int y = 0; y < 32; y++) {
                int y0 = bl_y_idx[y], y1 = y0 + 1, wy = bl_y_w[y];
                if (y1 >= src_h) y1 = src_h - 1;
                for (int x = 0; x < 32; x++) {
                    int x0 = bl_x_idx[x], x1 = x0 + 1, wx = bl_x_w[x];
                    ...
                    int top = (tl * (256 - wx) + tr * wx + 128) >> 8;
                    int bot = (bl * (256 - wx) + br * wx + 128) >> 8;
                    int pixel = (top * (256 - wy) + bot * wy + 128) >> 8;
