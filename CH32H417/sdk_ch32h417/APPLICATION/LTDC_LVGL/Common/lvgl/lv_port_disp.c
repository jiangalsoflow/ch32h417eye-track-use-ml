...
static uint16_t* display1 = layer1;
...
#else
        extern void GPHA_MixImg(uint32_t x, uint32_t y, uint32_t width, uint32_t height, uint32_t origin_width,
                                uint32_t origin_height, uint8_t* baseImg, uint8_t* fgImg);

        int32_t width  = area->x2 - area->x1 + 1;
        int32_t height = area->y2 - area->y1 + 1;

        GPHA_MixImg(area->x1, area->y1, width, height, MY_DISP_HOR_RES, MY_DISP_VER_RES, layer1, (void*)color_p);
