...
static void generate_report(void)
{
    ...
    printf("Weighted FPS: %"LV_PRIu32"\r\n", fps_weighted);
    printf("Opa. speed: %"LV_PRIu32"%%\r\n", opa_speed_pct);
...
static void report_cb(lv_timer_t * timer)
{
    ...
    if(opa_mode) {
        ...
        printf("Result of \"%s + opa\": %"LV_PRId32" FPS", scenes[scene_act].name,
               scenes[scene_act].fps_opa);
    }
    else {
        ...
        printf("Result of \"%s\": %"LV_PRId32" FPS", scenes[scene_act].name,
               scenes[scene_act].fps_normal);
    }
}
